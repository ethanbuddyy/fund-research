...
> 市场择时信号仅影响仓位比例（`composite_raw`），不再参与单基金排名（消除双重计算）。
...
`src/analyzers/narrative.py`：基于量化数据生成可读性文字观察，输出在 `signal["narrative"]["insights"]`。
覆盖四个维度：估值水位（CAPE / PE / ERP / 总市值-GDP 比）、市场情绪（VIX）、
...
**数据过期检测**：`provenance.check_staleness()` 检查各数据源最后更新时间，超期时在横幅中追加警告（宏观≤7天，行情≤3天，基金/估值≤7天）。调度器每次运行也会自动打印过期警告。
...
- ✅ 走向前回测引擎(无前视偏差，**四基准对比**，披露幸存者偏差)
- ✅ 每日定时调度(北京时区自适应，**信号变化自动通知**)
...
| 方向 | 说明 |
|------|------|
| **个人化输入** | 系统对所有用户输出相同建议；可加 `user_profile.yaml`（期限/风险偏好/现持仓），让仓位比例随个人情况偏移 |
| **回撤止损机制** | 当前无组合级浮亏保护；可加「持仓浮亏超 X% 时无论信号如何触发防守」的硬约束 |
...
- **四基准对比**：回测同时输出「等权买入持有」基准（无择时无择基），可直观区分信号择时与基金选择的各自贡献；若策略跑输等权买持，说明择时未带来超额收益。
- **live 与回测同口径**：信号权重、策略匹配、评分口径在实时与回测中保持一致（含类别相对化打分）。
